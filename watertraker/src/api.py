...
async def get_water_history(user_id:str):
    # strip surrounding quotes from URL parameter and ensure it's a string
    user_id_clean = str(user_id).strip().strip('"').strip("'")
    history= get_intake_history(user_id_clean)
    return {"user_id":user_id_clean,"history":history}